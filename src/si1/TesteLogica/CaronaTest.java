...
public class CaronaTest {
	...
	public void testOrigem() throws Exception {
		...
		try {
			c2 = new Carona(null, "Natal", "20/07/2012", "14:00", 2,usuario);
		} catch (Exception e) {
		...
		try {
			c3 = new Carona("", "Fortaleza", "04/08/2012", "09:00", 1,usuario);
		} catch (Exception e) {
			assertEquals("Origem inválida", e.getMessage());
		}
		
		try {
			c4 = new Carona("", "Fort@l&z@", "04/08/2012", "09:00", 1,usuario);
		} catch (Exception e) {
			assertEquals("Origem inválida", e.getMessage());
		}
	...
	public void testHora() throws Exception {
		...
		try{
			c3 = new Carona("Campina Grande", "Fortaleza", "04/08/2012", "09000", 1,usuario);	
		}catch(Exception e){
			assertEquals("Hora inválida", e.getMessage());
		}
		
		try{
			c4 = new Carona("Campina Grande", "Fortaleza", "04/08/2012", "009:0", 1,usuario);	
		}catch(Exception e){
			assertEquals("Hora inválida", e.getMessage());
		}
	}
